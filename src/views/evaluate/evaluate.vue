...
<!--        </el-select>-->
    ...
    <el-table :data="tableData" border fit style="width: 100%" v-loading="loading">
      <el-table-column label="月份" prop="orderDate">
      </el-table-column>
...
export default {
    methods: {
        handleCurrentChange() {
            GetAPI(`/api/product/listEvaluate/?page=${this.currentPage}&size=10`).then((data) => {
    ...
    created() {
        ...
        GetAPI(`/api/product/listEvaluate/?page=1&size=10`).then((data) => {
            this.tableData = data.data.data;
            this.tableData2 = data.data.data;
        })
      GetAPI(`/api/product/listEvaluate/?page=1&size=10000`).then((data) => {
